...
from db import (init_db, verify_user, add_material, log_qa, get_materials, add_user, delete_user, get_all_users, 
                ...
                assign_user_subject, remove_user_subject, create_quiz, assign_quiz_to_students, 
                get_teacher_quizzes, get_student_quizzes, get_quiz_by_id, update_quiz_score, get_db_connection)
        ...
        for file in files:
            if file and allowed_file(file.filename):
                ...
                if content.strip():
                    ...
                    add_material(filename, content, subject_id)
                else:
                    os.remove(file_path)
...
def quiz():
    ...
    if current_quiz:
        return render_template('quiz.html', quiz=current_quiz, show_quiz=True)
...
def dashboard():
    ...
    if user['role'] == 'admin':
        ...
        stats = get_teacher_stats(user['id'])
        recent_activities = get_teacher_recent_activities(user['id'])
    else:
        flash('Access denied.', 'error')
        return redirect(url_for('index'))
    ...
    subject_activity = get_subject_activity()
...
def get_recent_activities():
    ...
    cursor.execute('''
        SELECT u.username, qr.score, qr.time, q.title
...
def get_teacher_recent_activities(teacher_id):
    ...
    cursor.execute('''
        ...
        FROM quiz_results qr
        JOIN users u ON qr.user_id = u.id
        JOIN quiz_assignments qa ON qr.quiz_id = qa.quiz_id
...
def get_performance_data():
    ...
    return {
        ...
        'values': [high_scores, medium_scores, low_scores],
...
def get_subject_activity():
    ...
    cursor.execute('''
        SELECT s.name, COUNT(q.id) as quiz_count
        FROM subjects s
...
def get_time_series_data():
    ...
    cursor.execute('''
        SELECT DATE(time) as date, COUNT(*) as attempts
        FROM quiz_results 
    ...
    labels = []
    values = []
    for date, attempts in cursor.fetchall():
        labels.append(date)
